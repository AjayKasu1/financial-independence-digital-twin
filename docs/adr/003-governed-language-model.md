...
OpenRouter is an optional, server-side drafting provider. Requests use low temperature, strict JSON Schema output, compatible-endpoint routing, bounded output, minimal context, and provider data-collection denial. The synthetic-only `demo` environment may use a non-ZDR endpoint so zero-cost models remain available; development and production enforce zero-data-retention routing. Output is independently Zod-validated and policy-reviewed. Any failure uses a deterministic template.
